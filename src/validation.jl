...
function validate_regression(xyi,analysis_regression,xy_cv,value_cv)
    if isempty(value_cv)
        return NaN
    end

    #@show extrema(analysis_regression[.!isnan.(analysis_regression)])
    p = interpcv(xyi,analysis_regression,xy_cv)
